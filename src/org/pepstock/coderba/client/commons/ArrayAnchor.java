 ...
 * A simple wrapper around a homogeneous native array of anchor values.
 * 
 * @author Andrea "Stock" Stocchero
 */
@JsType(isNative = true, name = NativeName.ARRAY, namespace = JsPackage.GLOBAL)
public final class ArrayAnchor extends Array {

	/**
	 * This method creates new array instance with a variable number of <code>anchor</code> arguments.
	 * 
	 * @param items anchor items to create new array
	 * @return new array instance of anchors.
	 */
	private static native ArrayAnchor of(Anchor... items);
	 ...
	 * 
	 * @param items anchor items to create new array
	 * @return new array instance of anchors or <code>null</code> if argument is <code>null</code> or length to 0
	 */
	@JsOverlay
	public static ArrayAnchor fromOrNull(Anchor... items) {
	 ...
	 * 
	 * @param items anchor items to create new array
	 * @return new array instance of anchors or an empty array if argument is <code>null</code> or length to 0
	 */
	@JsOverlay
	public static ArrayAnchor fromOrEmpty(Anchor... items) {
		// checks if consistent
		if (items == null || items.length == 0) {
			// returns null
			return new ArrayAnchor();
		}
		// returns array
		return ArrayAnchor.of(items);
	}
	...
	/**
	 * Creates a java script array of anchors starting from list of anchors.
	 * 
	 * @param items list of anchors to load into new java script array.
	 * @return new array instance of anchors or <code>null</code> if argument is <code>null</code> or empty
	 */
	@JsOverlay
	public static ArrayAnchor fromOrNull(List<Anchor> items) {
		// checks if list is null
		if (items == null || items.isEmpty()) {
			return null;
		}
		// checks if is already a list with array
		if (items instanceof ArrayAnchorList) {
			// casts to array list
			ArrayAnchorList list = (ArrayAnchorList) items;
			// returns array
			return list.getArray();
		}
		// creates the array
		ArrayAnchor result = new ArrayAnchor();
		for (Anchor value : items) {
			// adds element
			result.push(value);
		}
		// returns the array
	 ...
	 * 
	 * @param items list of anchors to load into new java script array.
	 * @return new array instance of anchors or an empty array if argument is <code>null</code> or empty
	 */
	@JsOverlay
	public static ArrayAnchor fromOrEmpty(List<Anchor> items) {
		// checks if is already a list with array
		if (items instanceof ArrayAnchorList) {
			// casts to array list
			ArrayAnchorList list = (ArrayAnchorList) items;
	 ...
	 */
	native ArrayAnchor slice(int start, int end);
	 ...
	 */
	native ArrayAnchor splice(int start);
	 ...
	 *         If no elements are removed, an empty array is returned.
	 */
	native ArrayAnchor splice(int start, int deleteCounts);
	 ...
	 *         If no elements are removed, an empty array is returned.
	 */
	native ArrayAnchor splice(int start, int deleteCounts, Anchor item);
	 ...
	 */
	@JsOverlay
	void insertAt(int index, Anchor item) {
		splice(index, 0, item);
	}
	 ...
	 */
	native void fill(Anchor item, int start, int end);
	 ...
	 */
	@JsOverlay
	void set(int index, Anchor item) {
