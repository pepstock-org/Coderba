 ...
 * This implementation uses a java script array as back-end to store objects (positions).
 * 
 * @author Andrea "Stock" Stocchero
 * @see org.pepstock.coderba.client.commons.RegExpResult.client.commons.ArrayPosition
 */
public final class ArrayPositionList extends AbstractArrayList<Position, ArrayPosition> {

	// delegated array to store objects
	private final ArrayPosition array;
	 ...
	 */
	ArrayPositionList(ArrayPosition array) {
		// if null, creates a new array
		if (array == null) {
			this.array = new ArrayPosition();
		} else {
	 ...
	 */
	public void addAll(Position... values) {
		// checks if arguments are consistent
		if (values != null && values.length > 0) {
			// scans all elements
			for (Position val : values) {
				// adds
				add(val);
			}
		}
	 ...
	 */
	@Override
	public boolean add(Position element) {
	...
	public boolean addAll(Collection<? extends Position> collection) {
		...
		// checks if argument is consistent 
		if (modified) {
			Iterator<? extends Position> iter = collection.iterator();
			// scans all elements
			while (iter.hasNext()) {
				// adds and 
				// sets modified
	...
	public boolean retainAll(Collection<?> collection) {
		...
		if (modified) {
			...
			List<Position> contained = new ArrayList<>();
			...
			if (!contained.isEmpty()) {
				// scans all elements
				for (Position toRemove : contained) {
	...
	public Object[] toArray() {
		Position[] toArray = new Position[array.length()];
