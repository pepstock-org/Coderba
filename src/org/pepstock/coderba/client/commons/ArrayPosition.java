 ...
 * A simple wrapper around a homogeneous native array of position values.
 * 
 * @author Andrea "Stock" Stocchero
 */
@JsType(isNative = true, name = NativeName.ARRAY, namespace = JsPackage.GLOBAL)
public final class ArrayPosition extends Array {

	/**
	 * This method creates new array instance with a variable number of <code>position</code> arguments.
	 * 
	 * @param items position items to create new array
	 * @return new array instance of positions.
	 */
	private static native ArrayPosition of(Position... items);
	 ...
	 * 
	 * @param items position items to create new array
	 * @return new array instance of positions or <code>null</code> if argument is <code>null</code> or length to 0
	 */
	@JsOverlay
	public static ArrayPosition fromOrNull(Position... items) {
	 ...
	 * 
	 * @param items position items to create new array
	 * @return new array instance of positions or an empty array if argument is <code>null</code> or length to 0
	 */
	@JsOverlay
	public static ArrayPosition fromOrEmpty(Position... items) {
		// checks if consistent
		if (items == null || items.length == 0) {
			// returns null
	...
	/**
	 * Creates a java script array of positions starting from list of positions.
	 * 
	 * @param items list of positions to load into new java script array.
	 * @return new array instance of positions or <code>null</code> if argument is <code>null</code> or empty
	 */
	@JsOverlay
	public static ArrayPosition fromOrNull(List<Position> items) {
		// checks if list is null
		if (items == null || items.isEmpty()) {
			return null;
		}
		// checks if is already a list with array
		if (items instanceof ArrayPositionList) {
			// casts to array list
			ArrayPositionList list = (ArrayPositionList) items;
			// returns array
			return list.getArray();
		}
		// creates the array
		ArrayPosition result = new ArrayPosition();
		for (Position value : items) {
			// adds element
			result.push(value);
		}
		// returns the array
	 ...
	 * 
	 * @param items list of positions to load into new java script array.
	 * @return new array instance of positions or an empty array if argument is <code>null</code> or empty
	 */
	@JsOverlay
	public static ArrayPosition fromOrEmpty(List<Position> items) {
		// checks if is already a list with array
		if (items instanceof ArrayPositionList) {
			// casts to array list
			ArrayPositionList list = (ArrayPositionList) items;
	 ...
	 */
	native ArrayPosition slice(int start, int end);
	 ...
	 */
	native ArrayPosition splice(int start);
	 ...
	 *         If no elements are removed, an empty array is returned.
	 */
	native ArrayPosition splice(int start, int deleteCounts);
	 ...
	 *         If no elements are removed, an empty array is returned.
	 */
	native ArrayPosition splice(int start, int deleteCounts, Position item);
	 ...
	 */
	@JsOverlay
	void insertAt(int index, Position item) {
		splice(index, 0, item);
	}
	 ...
	 */
	native void fill(Position item, int start, int end);
	 ...
	 */
	native void push(Position item);
	 ...
	 */
	@JsOverlay
	void set(int index, Position item) {
